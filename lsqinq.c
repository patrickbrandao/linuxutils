...
int main(int argc, char **argv){
	...
	if(argc > 1){
		printf("\n");
		printf("lsqinq\n");
		printf("Lista de interfaces vlan 802.1ad\n");
		printf("\n");
	...
	if(send(rth.fd, (void*)&req, sizeof(req), 0)< 0 ) exit(1);
       ...
       if (rtnl_dump_filter(&rth, store_nlmsg, &linfo, NULL, NULL) < 0) {exit(1);}//Dump terminado    
        ...
        for (l=linfo; l; l = n) {
			...
			if( strncmp ( devname, "wlan", 4 ) == 0) show = 1;
			if( strncmp ( devname, "eth", 3 ) == 0) show = 1;
			if( strncmp ( devname, "ath", 3 ) == 0) show = 1;
			if( strncmp ( devname, "br", 2 ) == 0) show = 1;
			
			// se nao tiver ponto, ignorar
			if(strchr(devname, '.')==NULL) continue;

			// nao incluir qinq 802.1ad
			adp = strstr(devname, ".s");	
